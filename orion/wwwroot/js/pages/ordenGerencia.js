    ...
    fetch('/Orden/ListarOrdenes')
        ...
        .then(data => {
            if (Array.isArray(data)) {
                // Filtrar solo órdenes con estado 2
                const ordenesFiltradas = data.filter(orden => orden.idEstado === 2);
                gridApi.setGridOption('rowData', ordenesFiltradas);
...
async function editarOrden(id) {
    try {
        ...
        const proveedorSeleccionado = data.orden.proveedor;
        const idsEnOrden = new Set(data.productos.map(p => p.idDetalleSolicitud));

        const responseDetalles = await fetch(`/Orden/GetDetallesPorProveedor?proveedor=${encodeURIComponent(proveedorSeleccionado)}`);
